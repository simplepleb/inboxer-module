        ...
        <div class="row mt-4">
            <div class="col">
                <form action="{{ action('\Modules\Inboxer\Http\Controllers\Backend\TemplateController@store') }}" method="POST" class="ajax_upload_form form-validate-jquery">
                    ...
                    @include('inboxer::backend.templates._form')

                    {{--@include('inboxer::elements._tags', ['tags' => Modules\Inboxer\Entities\Template::tags()])--}}

                    <hr>
                    {{--<div class="text-right">
                        <button class="btn bg-teal mr-10"><i class="icon-check"></i> {{ trans('inboxer::messages.save') }}</button>
                        <a href="{{ action('\Modules\Inboxer\Http\Controllers\Backend\TemplateController@index') }}" class="btn bg-grey-800"><i class="icon-cross2"></i> {{ trans('inboxer::messages.cancel') }}</a>
                    </div>--}}
