...
abstract class CoreCampaign extends Model
{
    ...
    {
        if ($this->isAuto()) {
            return $this->autoEvent()->automation->belongsTo('Modules\Marketing\Entities\MailList', 'default_mail_list_id');
        } else {
            return $this->belongsTo('Modules\Marketing\Entities\MailList', 'default_mail_list_id');
        }
    }
    ...
    {
        return $this->belongsToMany('Modules\Marketing\Entities\MailList', 'mkt_campaigns_lists_segments');
    }
    ...
    {
        return $this->belongsTo('Modules\Marketing\Entities\SendingDomain', 'tracking_domain_id');
    }
    ...
    {
        return $this->belongsToMany('Modules\Marketing\Entities\AutoEvent', 'mkt_auto_campaigns');
    }
    ...
    {
        return $this->belongsToMany('Modules\Marketing\Entities\Link', 'mkt_campaign_links');
    }
    ...
    {
        return BounceLog::select('mkt_bounce_logs.*')->leftJoin('mkt_tracking_logs', 'mkt_tracking_logs.message_id', '=', 'mkt_bounce_logs.message_id')
            ->where('mkt_tracking_logs.campaign_id', '=', $this->id);
    }
    ...
    {
        return OpenLog::select('mkt_open_logs.*')->leftJoin('mkt_tracking_logs', 'mkt_tracking_logs.message_id', '=', 'mkt_open_logs.message_id')
            ->where('mkt_tracking_logs.campaign_id', '=', $this->id);
    }
    ...
    {
        return ClickLog::select('mkt_click_logs.*')->leftJoin('mkt_tracking_logs', 'mkt_tracking_logs.message_id', '=', 'mkt_click_logs.message_id')
            ->where('mkt_tracking_logs.campaign_id', '=', $this->id);
    }
    ...
    {
        return FeedbackLog::select('mkt_feedback_logs.*')->leftJoin('mkt_tracking_logs', 'mkt_tracking_logs.message_id', '=', 'mkt_feedback_logs.message_id')
            ->where('mkt_tracking_logs.campaign_id', '=', $this->id);
    }
    ...
    {
        return UnsubscribeLog::select('mkt_unsubscribe_logs.*')->leftJoin('mkt_tracking_logs', 'mkt_tracking_logs.message_id', '=', 'mkt_unsubscribe_logs.message_id')
            ->where('mkt_tracking_logs.campaign_id', '=', $this->id);
    }
    ...
    {
        return $this->hasMany('Modules\Marketing\Entities\CampaignsListsSegment');
    }
    ...
    public function prepareEmail($subscriber, $server)
    {
        ...
        $msgId = $customHeaders['X-Angie-Message-Id'];

        $message = new \Swift_Message();
        //$message = $mailon::newInstance();
        $message->setId($msgId);
    ...
    public static function resetMaxExecutionTime()
    {
        try {
            ...
            $arr = [ 'funct' => 'Emailer Message ', 'error_message' => 'Cannot reset max_execution_time: '.$e->getMessage()];
    ...
    {
        $arr = [ 'funct' => 'Emailer Message ', 'error_message' => 'isDone: '.$this->status];
    ...
    public function error($error = null)
    {
        ...
        $arr = [ 'funct' => 'Emailer Message ', 'error_message' => 'isError: '.$this->last_error];
    ...
    public function getCustomHeaders($subscriber, $server)
    {
        ...
        return array(
            'X-Angie-Campaign-Id' => $this->uid,
            'X-Angie-Subscriber-Id' => $subscriber->uid,
            'X-Angie-Customer-Id' => $this->customer->uid,
            'X-Angie-Message-Id' => $msgId,
            'X-Angie-Sending-Server-Id' => $server->uid,
            'X-No-Track' => $subscriber->uid, // Stops mailtracker-package tracking @todo Angie
    ...
    {
        return self::select('mkt_campaigns.*');
    }
    ...
    public function step()
    {
        ...
        if (is_object($this->subscribers([], ['mkt_email_verifications'])->limit(1)->first())) {
            $step = 5;
        } else {
            return $step;
        }
    ...
    public function log($name, $customer, $add_datas = [])
    {
        ...
        \Modules\Marketing\Entities\Log::create([
    ...
    public function clickCount($start = null, $end = null)
    {
        ...
        if (isset($start)) {
            $query = $query->where('mkt_click_logs.created_at', '>=', $start);
        }
        if (isset($end)) {
            $query = $query->where('mkt_click_logs.created_at', '<=', $end);
        }
    ...
    public function openUniqCount($start = null, $end = null)
    {
        $query = $this->openLogs();
        if (isset($start)) {
            $query = $query->where('mkt_open_logs.created_at', '>=', $start);
        }
        if (isset($end)) {
            $query = $query->where('mkt_open_logs.created_at', '<=', $end);
        }
    ...
    {
        return \Modules\Marketing\Web\Subscriber::selectRaw(DB::getTablePrefix().'mkt_list_subscriber.*, COUNT('.DB::getTablePrefix().'mkt_campaign_track_unsubscribe.id) AS openCount')
                            ->leftJoin('mkt_campaign_track_unsubscribe', 'mkt_campaign_track_unsubscribe.subscriber_id', '=', 'mkt_list_subscriber.subscriber_id')
                            ->where('mkt_campaign_track_unsubscribe.campaign_id', '=', $this->campaign_id)
                            ->groupBy('mkt_list_subscriber.subscriber_id')
    ...
    {
        $records = self::select(DB::raw('mkt_campaigns.*, count(*) as `aggregate`'))
            ->join('mkt_tracking_logs', 'mkt_tracking_logs.campaign_id', '=', 'mkt_campaigns.id')
            ->join('mkt_open_logs', 'mkt_open_logs.message_id', '=', 'mkt_tracking_logs.message_id')
            ->where('is_auto', false);

        if (isset($customer)) {
            $records = $records->where('mkt_campaigns.customer_id', '=', $customer->id);
        }

        $records = $records->groupBy('mkt_campaigns.id')
    ...
    public static function topClicks($number = 5, $customer = null)
    {
        $records = self::select(DB::raw('mkt_campaigns.*, count(*) as `aggregate`'))
            ->join('mkt_tracking_logs', 'mkt_tracking_logs.campaign_id', '=', 'mkt_campaigns.id')
            ->join('mkt_click_logs', 'mkt_click_logs.message_id', '=', 'mkt_tracking_logs.message_id')
            ->where('is_auto', false);
    ...
    {
        $records = Link::select(DB::raw('mkt_links.*, count(*) as `aggregate`'))
            ->join('mkt_campaign_links', 'mkt_campaign_links.link_id', '=', 'mkt_links.id')
            ->join('mkt_tracking_logs', 'mkt_tracking_logs.campaign_id', '=', 'mkt_campaign_links.campaign_id')
            ->join('mkt_click_logs', function ($join) {
                $join->on('mkt_click_logs.message_id', '=', 'mkt_tracking_logs.message_id')
                ->on('mkt_click_logs.url', '=', 'mkt_links.url');
            });

        if (isset($customer)) {
            $records = $records->join('mkt_campaigns', 'mkt_campaign_links.campaign_id', '=', 'mkt_campaigns.id')
                ->where('mkt_campaigns.customer_id', '=', $customer->id);
        }

        $records = $records->groupBy('mkt_links.id')
            ->orderBy('aggregate', 'desc');
    ...
    {
        $records = ClickLog::select(DB::raw('mkt_click_logs.*, count(*) as `aggregate`'))
            ->leftJoin('mkt_tracking_logs', 'mkt_tracking_logs.message_id', '=', 'mkt_click_logs.message_id')
            ->where('mkt_tracking_logs.campaign_id', '=', $this->id);

        $records = $records->groupBy('mkt_click_logs.url')
            ->orderBy('aggregate', 'desc');
    ...
    {
        $records = Subscriber::select(DB::raw('mkt_subscribers.*, count(*) as `aggregate`'))
            ->join('mkt_tracking_logs', 'mkt_tracking_logs.subscriber_id', '=', 'mkt_subscribers.id')
            ->join('mkt_open_logs', 'mkt_open_logs.message_id', '=', 'mkt_tracking_logs.message_id')
            ->where('campaign_id', '=', $this->id);

        $records = $records->groupBy('mkt_tracking_logs.message_id')
            ->orderBy('aggregate', 'desc');
    ...
    {
        $records = IpLocation::select(DB::raw('ip_locations.*, count(*) as `aggregate`'))
            ->join('mkt_open_logs', 'mkt_open_logs.ip_address', '=', 'ip_locations.ip_address')
            ->join('mkt_tracking_logs', 'mkt_open_logs.message_id', '=', 'mkt_tracking_logs.message_id')
            ->where('mkt_tracking_logs.campaign_id', '=', $this->id);

        if (isset($customer)) {
            $records = $records->join('mkt_campaigns', 'mkt_tracking_logs.campaign_id', '=', 'mkt_campaigns.id')
                ->where('mkt_campaigns.customer_id', '=', $customer->id);
        }

        $records = $records->groupBy('mkt_open_logs.ip_address')
            ->orderBy('aggregate', 'desc');
    ...
    {
        $records = IpLocation::select(DB::raw('ip_locations.*, count(*) as `aggregate`'))
            ->join('mkt_click_logs', 'mkt_click_logs.ip_address', '=', 'ip_locations.ip_address')
    ...
    {
        $records = IpLocation::select('ip_locations.*', 'mkt_open_logs.created_at as open_at', 'mkt_subscribers.email as email')
            ->leftJoin('mkt_open_logs', 'mkt_open_logs.ip_address', '=', 'ip_locations.ip_address')
            ->leftJoin('mkt_tracking_logs', 'mkt_open_logs.message_id', '=', 'mkt_tracking_logs.message_id')
            ->leftJoin('mkt_subscribers', 'mkt_subscribers.id', '=', 'mkt_tracking_logs.subscriber_id')
            ->where('mkt_tracking_logs.campaign_id', '=', $this->id);
    ...
    public function inlineHtml($html)
    {
        ...
        if ($this->template_source == 'builder') {
            ...
            $css = file_get_contents(public_path('/assets/modules/marketing/public/css/res_email.css'));
    ...
    public function sendTestEmail($email)
    {
        try {
            ...
            return [
                'status' => 'success',
                'message' => trans('marketing::messages.campaign.test_sent'),
            ];
        } catch (\Exception $e) {
            return [
                'status' => 'error',
    ...
    {
        return [
            ['text' => trans('marketing::messages.'.self::TYPE_REGULAR), 'value' => self::TYPE_REGULAR],
            ['text' => trans('marketing::messages.'.self::TYPE_PLAIN_TEXT), 'value' => self::TYPE_PLAIN_TEXT],
        ];
    }
    ...
    public function subscribersCount($cache = false)
    {
        ...
        return distinctCount($this->subscribers([], ['mkt_email_verifications']), 'mkt_subscribers.email');
    }
    ...
    {
        return distinctCount($this->subscribers([], ['mkt_email_verifications'])->where('mkt_subscribers.status', Subscriber::STATUS_SUBSCRIBED), 'subscribers.email');
    }
    ...
    {
        $query = $this->openLogs()->select('mkt_open_logs.created_at');
        if (isset($start)) {
            $query = $query->where('mkt_open_logs.created_at', '>=', $start);
        }
        if (isset($end)) {
        ...
        return $query->orderBy('mkt_open_logs.created_at', 'asc')->get()->groupBy(function ($date) {
            return \Modules\Marketing\Library\Tool::dateTime($date->created_at)->format('H'); // grouping by hours
        });
    }
    ...
    {
        $query = $this->clickLogs()->select('mkt_click_logs.created_at', 'mkt_tracking_logs.subscriber_id');
        ...
        return $query->orderBy('mkt_click_logs.created_at', 'asc')->get()->groupBy(function ($date) {
            return \Modules\Marketing\Library\Tool::dateTime($date->created_at)->format('H'); // grouping by hours
        });
    }
    ...
    public function fillAttributes($params)
    {
        ...
        if (isset($params['tracking_domain_uid'])) {
            $tracking_domain = \Modules\Marketing\Entities\SendingDomain::findByUid($params['tracking_domain_uid']);
