<?php
    $label = isset($label) ? $label : (Lang::has('inboxer::messages.'.$name) ? trans('inboxer::messages.'.$name) : '');
...
@if ($type == 'checkbox')
    @include('inboxer::backend.common._' . $type)
@elseif (!empty($errors))
    <div class="form-group{{ $errors->has($var_name) ? ' has-error' : '' }} control-{{ $type }}">
        @if (!empty($label) && $type != 'checkbox2')
            <label>
                ...
                @if (isset($check_all_none))
                    &nbsp;&nbsp;&nbsp;
                    <a href="#all" class="checkboxes_check_all">{{ trans('inboxer::messages.all') }}</a>
                    | <a href="#none" class="checkboxes_check_none">{{ trans('inboxer::messages.none') }}</a>
                @endif
            </label>
        @endif
        ...
        @if (isset($help_class) && Lang::has('inboxer::messages.' . $help_class . '.' . $name . '.help'))
            <div class="help alert alert-info">
                {!! trans('inboxer::messages.' . $help_class . '.' . $name . '.help') !!}
            </div>
        @endif
