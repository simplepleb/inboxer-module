...
class QuotaTrackerFile
{
    ...
    public function __construct($file, $quota = null, $limits = [])
    {
        ...
        $this->file_path = storage_path('/logs/'); // Storage::disk("tenant")->getDriver()->getAdapter()->getPathPrefix();
    ...
    private function createLogFile()
    {
        ...
        if (!file_exists($this->file_path.'lock')) {
            touch($this->file_path.'lock');
            chmod($this->file_path.'lock', 0777);
        }
        $fp = fopen($this->file_path.'lock', "r+");
        $start = time();
        while (true) {
            $this->timeout($start);
